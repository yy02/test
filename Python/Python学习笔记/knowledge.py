...
def changeGlobal():
    ...
    :return:
    '''
    # global pro # 此处global关键字的作用是指明此处的局部变量为全局变量，这样就可以对全局变量进行修改
    pro = "mmmmmm"  # 定义局部变量
...
enumerate() #用于将一个可遍历的数据对象同时取出数据和数据下标，一般用在for循环中
listobj = ['a','b','c']
...
set集合：
set集合是以无序不重复的数据集合，不支持索引和切片，和字典类似

创建set集合的两种方式：
1. set = {1,2,3}  #和字典一样的直接定义
2.list = [1,2,3,5,4]    
    set1 = set(list) #强制转换
...
add()   #添加操作
clear()  #清空集合操作
difference()    #取差集，set1集合中有而set2集合中没有
                    例如set1.difference(set2)或者print(set1-set2)
intersection()  #取交集   print(set1&set2)
union()     #取并集
pop()       #从集合中拿数据同时删除
disca()     #移除指定数据
update()    #两个集合，set1.update(set2) 将set2添加到set1中
